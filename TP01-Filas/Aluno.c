...
void alu_atribui(Aluno *aluno, int matricula, char *nome, char *curso) {
  if (aluno != NULL && strlen(nome) <= 50 && strlen(curso) <= 30 &&
      matricula >= 0) {
 ...
 * possuem os mesmos dados, 0 caso os dados dos alunos possuam alguma
 * diferença e -1 caso pelo menos um dos alunos seja NULL.
 */
